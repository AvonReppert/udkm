...
def regridArray(Array, Column, Grid, RemoveColumns):
    ...
    # This temparry array only contains the remaining columns
    ArrayTemp = np.delete(Array, RemoveColumns, 1)
    ColsNew = np.size(ArrayTemp, 1)
    # This is the new Array that will eventually be returned
    ArrayNew = np.zeros((np.size(Grid)-1, ColsNew*2))
