...
def import_file(sample='LiNbO3', sliced = False, power='10', fwhm_x= '830', fwhm_y='60' , parameters_example =  'verlauf'):
    path= r'\\141.89.115.120\Measurement\FemtoMag-TG\TG_FROG\\'
    filename = askopenfilename(initialdir=path)
    wl=np.genfromtxt(r'\\141.89.115.120\Samples\LiNbO3\JohannesProject\Programme\wavelength.txt')
    data=np.genfromtxt(filename)
    #prepare data, to have w<avelength, delay and 2D data in one matrix we define data_ready
    delay=data[:,0]  
    data_ready=np.zeros([len(delay)+1,len(wl)+1])
    data_ready[1:,1:]=data[0:,2:]
    data_ready[0,1:]=wl
    data_ready[1:,0]=delay
    data=data_ready
    ind = np.unravel_index(np.argmax(data, axis=None), data.shape)
    data_2D=data[1:,1:]
    data_2D[data_2D<1] = 1
    data_dict = { 'wl' :wl, 'data' :data, 'delay' :delay, 'data_2D' :data_2D, 'ind' :ind, 'sliced' : sliced }
    filename_parameter = r'C:\Users\johannmuelle\Code\udkm\udkm\tg\example\parameters'+'\\'+'parameters_'+parameters_example+'.txt'
    f=open(filename_parameter,'a')
    
    date=filename[len('\\141.89.115.120\Measurement\FemtoMag-TG\TG_FROG\\')+1:len('\\141.89.115.120\Measurement\FemtoMag-TG\TG_FROG\\')+9]
    time=filename[len('\\141.89.115.120\Measurement\FemtoMag-TG\TG_FROG\\')+10:len('\\141.89.115.120\Measurement\FemtoMag-TG\TG_FROG\\')+16]
    outfile_name= r'C:\Users\johannmuelle\Code\udkm\udkm\tg\example\scan_export\\'+str(date)+'_'+str(time)+'.pickle'
    pickle.dump(data_dict, open(outfile_name, 'wb'))
    f.write(str(date)+'\t'+str(time)+'\t'+str(fwhm_x)+'\t'+str(fwhm_y)+'\t'+str(power)+'\t'+str(sample)+'\n')
    f.close()
    
    
def plot_overview(line, parameters_example = 'verlauf',Fourier_Analyse=False, Fit_Analyse=False):
    parameter = pd.read_csv(r'C:\Users\johannmuelle\Code\udkm\udkm\tg\example\parameters'+'\\'+'parameters_'+parameters_example+'.txt', skiprows=1, sep='\t')
    date = parameter.date[line]
    time = parameter.time[line]
    infile = open(r'C:\Users\johannmuelle\Code\udkm\udkm\tg\example\scan_export'+'\\'+str(date)+'_'+str(time)+'.pickle','rb')
    ...
    if Fit_Analyse==True:
        model = lm.models.GaussianModel(prefix = "g1_")+ lm.models.LinearModel() #Initializes the fit model as the sum of a gaussian with a background
    ...
    if Fourier_Analyse==True:
        ax2 = plt.subplot(gs[1])
        ax2.semilogy(f,fouriersumme,label=str(round_it(f[indfourier[1]+5],2))+' THz at '+str(round_it(fouriersumme[np.argmax(fourier_2D[1,5:maximumfinder])+5],1)*0.001)+' [x 10$^3$ a.u.]')
    ...
    pl =ax3.pcolormesh(X,Y,np.transpose(data_2D),vmin =data_2D.min(),vmax = data_2D.max(),norm=matplotlib.colors.LogNorm(),cmap=cmap)
    ...
    axins3 = inset_axes(ax3,
                        ...
                        height="5%", # height : 50%                   )
                        loc = 4)
    ax3.add_patch(Rectangle((0.35,0.018), 0.65, 0.17, edgecolor = "none",facecolor="white",alpha = 0.90,transform=ax3.transAxes))
    ...
    plt.savefig(r'C:\Users\johannmuelle\Code\udkm\udkm\tg\example\plot_overview'+'\\'+str(title)+'.png',dpi = 300)
