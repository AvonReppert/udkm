...
def read_data_rss(scan_dict, bool_refresh_data, data_path):
    ...
    data_raw = np.genfromtxt('RawData/' + str(get_label(ref_file, number)
                             [1]) + '/scans' + str(read_param(ref_file, number)[1]) + '.dat', comments='%')
...
def get_label(ref_file, number):
    ...
    """
    plot_title = str(number) + ' | ' + read_param(ref_file, number)[0] + ' | ' + read_param(ref_file, number)[
        1] + ' | ' + str(read_param(ref_file, number)[2]) + 'K' + ' | ' + str(read_param(ref_file, number)[3]) + 'mJ/cm2'
...
def plot_rocking_overview(ref_file, number, rocking, qz_range, delay_steps):
    ...
    plt.savefig('RawData/' + get_label(ref_file, number)
                [1] + '/exportedFigures/rockingCurves2Dplot.png', dpi=400, bbox_inches='tight')
    plt.show()
...
def plot_peak_fit(ref_file, number, qz_roi, fit, ref_fit, delay):
    ...
    plt.savefig('RawData/' + get_label(ref_file, number)
                [1] + '/Fitgraphics' + '/'+str(int(10*delay))+'ps.png', dpi=200, bbox_inches='tight')
...
def plot_transient_results(ref_file, number, transient_results, delay_steps):
    ...
    plt.text(0.25, 1.16, get_label(ref_file, number)[0] + str(read_param(ref_file, number)[3]) + 'K     ' + str(
        read_param(ref_file, number)[3]) + 'mJ/cm2', fontsize=16, ha='left', va='center', transform=ax1.transAxes)
